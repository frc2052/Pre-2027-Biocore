...
public class PositionEstimator {
  ...
  /**
   * Updates the pose estimate via a set of {@link PoseMeasurement} and a set of {@link
   * OdometryMeasurement}.
   *
   ...
   */
  public void update(
      PoseMeasurement[] visionMeasurements, OdometryMeasurement[] odometryMeasurements) {
    ...
    for (OdometryMeasurement om : odometryMeasurements) {
      if (!odometryLastPose.containsKey(om.name)) {
        ...
        double theta =
            (om.measurement.pose.getRotation().getRadians() - lastPose.getRotation().getRadians())
        ...
        totalMeasurements.add(
            new PoseMeasurement(
                actualMeasuredPose,
                newStdDev,
                om.measurement.measurementDelay,
                om.measurement.measurementReceivedTimestamp));
      }
  ...
  private void applyOdometryUpdates() {
    for (String name : toUpdate) {
      ...
      Pair<Pose2d, Optional<Pair<Pose2d, Vector<N3>>>> newPair =
          new Pair<>(mapPair.getFirst(), Optional.of(new Pair<>(lastPose, lastPoseStdDev)));
  ...
  private void applyAllMeasurements(PoseMeasurement... measurements) {
    ...
    if (measurements.length != 0) {
      ...
      allX.add(new Pair<>(newPredictionJump.pose.getX(), newPredictionJump.poseStdDev.get(0)));
      allY.add(new Pair<>(newPredictionJump.pose.getY(), newPredictionJump.poseStdDev.get(1)));
      allTheta.add(
          new Pair<>(
              newPredictionJump.pose.getRotation().getRadians(),
              newPredictionJump.poseStdDev.get(2)));
      ...
      Vector<N3> newPoseStandardDeviations =
          VecBuilder.fill(x.getSecond(), y.getSecond(), theta.getSecond());
      ...
      lastUpdateHadValidMeasurements = true;
    } else {
      lastPose = newPredictionJump.pose;
      lastPoseStdDev = newPredictionJump.poseStdDev;
      lastVelocity = newPredictionJump.velocity;
      lastVelocityStdDev = newPredictionJump.velocityStdDev;
      lastAcceleration = newPredictionJump.acceleration;
      lastAccelerationStdDev = newPredictionJump.accelerationStdDev;
      lastJerk = newPredictionJump.jerk;
      lastJerkStdDev = newPredictionJump.jerkStdDev;
      lastUpdateHadValidMeasurements = false;
    }

    lastPredictionJump = newPredictionJump;
    if (lastUpdateHadValidMeasurements) {
      lastPredictionError = new Pose2d();
    } else {
      lastPredictionError =
          new Pose2d(
              lastPose.getX() - lastPredictionJump.pose.getX(),
              lastPose.getY() - lastPredictionJump.pose.getY(),
              new Rotation2d(
                  lastPose.getRotation().getRadians()
                      - lastPredictionJump.pose.getRotation().getRadians()));
    }
  }
   ...
   *
   * @param distributionPairs Pair where the first is the mean and the second is the standard
   *     deviation.
  ...
  /**
   * Predicts the future pose of the robot based on its current estimation. Considers the first
   * derivative (velocity) as constant.
   *
  ...
  /**
   * Predicts the future pose of the robot based on its current estimation. Considers the second
   * derivative (acceleration) as constant.
   *
   * @param timestep timestep into the future to predict too
   * @return the {@link PosePrediction} of the prediction.
   */
  public PosePrediction predictDTwo(Time timestep) {
    ...
    ChassisVelocities newVel =
        lastVelocity.plus(
            new ChassisVelocities(
                lastAcceleration.ax * dt, lastAcceleration.ay * dt, lastAcceleration.alpha * dt));
    ...
    Pose2d newPose2d =
        new Pose2d(
            ...
            new Rotation2d(
                lastPose.getRotation().getRadians()
                    + lastVelocity.omega * dt
                    + lastAcceleration.alpha * dtsq));
    ...
    Vector<N3> newPoseStdDev =
        VecBuilder.fill(
            lastPoseStdDev.get(0)
                + lastVelocityStdDev.get(0) * dt
                + lastAccelerationStdDev.get(0) * dtsq,
            lastPoseStdDev.get(1)
                + lastVelocityStdDev.get(1) * dt
                + lastAccelerationStdDev.get(1) * dtsq,
            lastPoseStdDev.get(2)
                + lastVelocityStdDev.get(3) * dt
                + lastAccelerationStdDev.get(2) * dtsq);
  ...
  /**
   * Predicts the future pose of the robot based on its current estimation. Considers the third
   * derivative (jerk, don't be one) as constant.
   *
   * @param timestep timestep into the future to predict too
   * @return the {@link PosePrediction} of the prediction.
   */
  public PosePrediction predictDThree(Time timestep) {
    ...
    ChassisAccelerations newAcc =
        lastAcceleration.plus(
            new ChassisAccelerations(lastJerk.jx * dt, lastJerk.jy * dt, lastJerk.zeta * dt));
    ChassisVelocities newVel =
        lastVelocity.plus(
            new ChassisVelocities(
                lastAcceleration.ax * dt + lastJerk.jx * dtsq,
                lastAcceleration.ay * dt + lastJerk.jy * dtsq,
                lastAcceleration.alpha * dt + lastJerk.zeta * dtsq));

    Pose2d newPose2d =
        new Pose2d(
            lastPose.getX()
                + lastVelocity.vx * dt
                + lastAcceleration.ax * dtsq
                + lastJerk.jx * dtcb,
            lastPose.getY()
                + lastVelocity.vy * dt
                + lastAcceleration.ay * dtsq
                + lastJerk.jy * dtcb,
            new Rotation2d(
                lastPose.getRotation().getRadians()
                    + lastVelocity.omega * dt
    ...
    Vector<N3> newVelStdDev =
        VecBuilder.fill(
            lastVelocityStdDev.get(0)
                + lastAccelerationStdDev.get(0) * dt
                + lastJerkStdDev.get(0) * dtsq,
            lastVelocityStdDev.get(1)
                + lastAccelerationStdDev.get(1) * dt
                + lastJerkStdDev.get(1) * dtsq,
            lastVelocityStdDev.get(2)
                + lastAccelerationStdDev.get(2) * dt
                + lastJerkStdDev.get(2) * dtsq);
    ...
    Vector<N3> newPoseStdDev =
        VecBuilder.fill(
            lastPoseStdDev.get(0)
                + lastVelocityStdDev.get(0) * dt
                + lastAccelerationStdDev.get(0) * dtsq
                + lastJerkStdDev.get(0) * dtcb,
            lastPoseStdDev.get(1)
                + lastVelocityStdDev.get(1) * dt
                + lastAccelerationStdDev.get(1) * dtsq
                + lastJerkStdDev.get(1) * dtcb,
            lastPoseStdDev.get(2)
                + lastVelocityStdDev.get(3) * dt
                + lastAccelerationStdDev.get(2) * dtsq
                + lastJerkStdDev.get(2) * dtcb);
  ...
  /**
   * Compensates for time delay by increasing the standard deviation for latency via the motion of
   * the robot.
   *
  ...
  private PoseMeasurement compensateForTime(PoseMeasurement m) {
    ...
    Vector<N3> newPoseStdDev =
        VecBuilder.fill(
            m.poseStdDev.get(0)
                + lastVelocityStdDev.get(0) * delaySeconds
                + lastAccelerationStdDev.get(0) * dsq
                + lastJerkStdDev.get(0) * dcb,
            m.poseStdDev.get(1)
                + lastVelocityStdDev.get(1) * delaySeconds
                + lastAccelerationStdDev.get(1) * dsq
                + lastJerkStdDev.get(1) * dcb,
            m.poseStdDev.get(2)
                + lastVelocityStdDev.get(2) * delaySeconds
                + lastAccelerationStdDev.get(2) * dsq
                + lastJerkStdDev.get(2) * dcb);

    return new PoseMeasurement(
        m.pose, newPoseStdDev, m.measurementDelay, m.measurementReceivedTimestamp);
  }
  ...
  public class PoseMeasurement {
    ...
    public final Time
        measurementReceivedTimestamp; // timestamp for when software received the pose, is used for

    // validation and updating delay.

    public PoseMeasurement(
        Pose2d pose,
        Vector<N3> poseStdDev,
        Time measurementDelay,
        Time measurementReceivedTimestamp) {
      this.pose = pose;
      this.poseStdDev = poseStdDev;
