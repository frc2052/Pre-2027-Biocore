 ...
 *
 * <p>https://docs.limelightvision.io/docs/docs-limelight/apis/complete-networktables-api
 */
public class LimelightCamera {
   ...
   *
   * @return The horizontal offset from crosshair to target (LL1: -27 degrees to 27 degrees / LL2:
   ...
   *
   * @return The vertical offset from crosshair to target (LL1: -20.5 degrees to 20.5 degrees / LL2:
   ...
   *
   * @return The total latency (ms). Time between the end of the exposure of the middle row of the
   *     sensor to the beginning of the tracking pipeline plus the pipeline’s latency contribution.
   */
   ...
   *
   * @return Array containing several values for matched-timestamp statistics: [targetValid,
   *     targetCount, targetLatency, captureLatency, tx, ty, txnc, tync, ta, tid,
   *     targetClassIndexDetector , targetClassIndexClassifier, targetLongSidePixels,
   ...
   *
   * @return Full JSON dump of targeting results. Must be enabled per-pipeline in the 'output' tab
   *     of the Limelight web UI.
   */
   ...
   *
   * @return LimelightBotPose object containing the robot's pose, latency, tag count, tag span,
   *     average tag distance from camera, and average tag area.
   */
  @MegaTag1
  ...
  public LimelightBotPose getBotPose_Orb() {
    return botPoseArrayToLimelightBotPose(
        table.getEntry("botpose_orb").getDoubleArray(new double[11]));
  }
  ...
  public LimelightBotPose getBotPose_Orb_wpiBlue() {
    return botPoseArrayToLimelightBotPose(
        table.getEntry("botpose_orb_wpiblue").getDoubleArray(new double[11]));
  }
  ...
  public LimelightBotPose getBotPose_Orb_wpiRed() {
    return botPoseArrayToLimelightBotPose(
        table.getEntry("botpose_orb_wpired").getDoubleArray(new double[11]));
  }
   ...
   *
   * @return MegaTag Standard Deviations [MT1x, MT1y, MT1z, MT1roll, MT1pitch, MT1Yaw, MT2x, MT2y,
   *     MT2z, MT2roll, MT2pitch, MT2yaw].
   */
   ...
   *
   * @return An array of RawFiducial objects representing the raw fiducials detected by the
   *     Limelight camera.
   */
   ...
   *
   * @return An array of RawDetection objects representing the raw detections detected by the
   *     Limelight camera.
   */
   ...
   *
   * @return An array of RawTarget objects representing the raw targets detected by the Limelight
   *     camera.
   */
   ...
   *
   * @param getter Function that takes a String (Limelight camera name) and returns a value of type
   *     {@link T}.
  ...
  /**
   * Recommend setting this to 100-200 while disabled. Sets number of frames to skip between
   * processed frames to reduce temperature rise. Outputs are not zeroed during skipped frames.
   *
   * @param throttle The number of frames to skip between processed frames. 0 means process every
   *     frame, 1 means process every other frame, etc.
   */
  ...
  /**
   * Override valid fiducial ids for localization. This is useful if you want to ignore certain tags
   * or only use a subset of tags for localization.
   *
  ...
  private Pose3d botPoseArrayToPose3d(double[] botPoseArray) {
    ...
    return new Pose3d(
        new Translation3d(
            Meters.of(botPoseArray[0]), Meters.of(botPoseArray[1]), Meters.of(botPoseArray[2])),
        new Rotation3d(
            Degrees.of(botPoseArray[3]), Degrees.of(botPoseArray[4]), Degrees.of(botPoseArray[5])));
  }
   ...
   *
   * @param botPoseArray The botpose array from the Limelight camera.
   * @return A LimelightBotPose object representing the robot's pose, latency, tag count, tag span,
   *     average tag distance from camera, and average tag area.
  ...
  private LimelightBotPose botPoseArrayToLimelightBotPose(double[] botPoseArray) {
    if (botPoseArray.length != 11) {
      System.err.println(
          "Warning: botPose array length is not 11. Returning default LimelightBotPose.");
      return new LimelightBotPose(
          ...
          0.0,
          Meters.of(0),
          0.0); // Return a default LimelightBotPose if the array length is not 11
    }
    return new LimelightBotPose(
